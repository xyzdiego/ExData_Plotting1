...
sub_hpower$Voltage <- as.numeric(as.character(sub_hpower$Voltage))
sub_hpower$Global_reactive_power <- as.numeric(as.character(
    sub_hpower$Global_reactive_power))
str(sub_hpower)
summary(sub_hpower)
...
with({
    ...
    with(sub_hpower, plot(Time, Voltage, type = "l", 
    ...
    legend("topright", lty = 1, col = c("black", "red", "blue"), cex = 0.55,
           legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
    with(sub_hpower, plot(Time, Global_reactive_power, type = "l",
